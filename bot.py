...
class PonchBot:
    ...
    def _build_probability_breakdown_answer(self, text, fallback_symbol=None):
        ...
        lines = [
            ...
            f"Funding: {self._fmt_funding_pct(payload.get('funding_rate'), decimals=4, already_percent=True)} ({str(funding_ctx.get('bias') or 'flat')})",
    ...
    def _build_no_trade_answer(self, text, fallback_symbol=None):
        ...
        lines = [
            ...
            f"<blockquote>Price: {current_price:,.2f} | Funding: {self._fmt_funding_pct(payload.get('funding_rate'), decimals=4, already_percent=True)}</blockquote>",
            "Main blockers:",
        ]
    ...
    def _send_liquidation_map_post(self, *, chat_id=None, message_thread_id=None):
        ...
        message = (
            ...
            f"Funding: {self._fmt_funding_pct(snapshot.get('funding_rate'), decimals=4, already_percent=True)}"
            f"</blockquote>\n"
    ...
    def _build_session_game_plan_message(self, session_name, latest_price=None):
        ...
        funding_map = dict(multi_ctx.get("funding") or {})
        funding_bits = []
        funding_bits.append(f"Bitunix {self._fmt_funding_pct(funding_rate, decimals=4, already_percent=True)}")
        bitget_rate = funding_map.get("bitget")
        if bitget_rate is not None:
        ...
        if not funding_bits:
            funding_text = f"{self._fmt_funding_pct(funding_rate, decimals=4, already_percent=True)} ({html.escape(funding_bias)})"
        else:
    ...
    def _build_symbol_chart_chat_answer(self, text, fallback_symbol=None, mode=None, style="normal"):
        ...
        lines = [
            ...
            (
                "<blockquote>"
                f"Price: {current_price:,.2f}\n"
                f"Funding: {self._fmt_funding_pct(funding_rate, decimals=4, already_percent=True)} ({funding_bias})\n"
                f"1H: {bias_1h} | 4H: {bias_4h} | 1D: {bias_1d}\n"
                f"24H: {day_change:+.2f}%"
                "</blockquote>"
            ),
    ...
    @staticmethod
    def _fmt_funding_pct(rate, decimals=4, already_percent=False):
        try:
