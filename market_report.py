...
def _fmt_funding_pct(value, decimals=4, already_percent=False):
...
def _scenario_mode_config(mode):
    ...
    if mode_name == "short_term":
        return {
            ...
            "near_pick_min_usd": 0.0,
            "near_pick_max_usd": 1500.0,
            "near_pick_target_usd": 800.0,
...
def build_btc_market_report(symbol=SYMBOL, mode="swing"):
    ...
    blocks = [
        (
            ...
            f"Funding: {_fmt_funding_pct(funding_rate_raw, already_percent=True)}"
            "</blockquote>"
        ),
    ]
